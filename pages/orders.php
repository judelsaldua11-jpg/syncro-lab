...
/* ── Order items (batched into one query) ──────────────────── */
$orderDetails = [];
if ($orders) {
...
/* ── Status → timeline step map ────────────────────────────── */
const ORDER_STEPS   = ['Order Placed', 'Confirmed', 'Shipped', 'Delivered'];
const STATUS_STEPS  = [
    'pending'   => 0,
    'confirmed' => 1,
    'shipped'   => 2,
...
<style>
    ...
    .order-items-toggle { cursor: pointer; color: var(--green); font-weight: 600; font-size: 14px; transition: color 0.2s ease; }
    ...
    .reorder-btn {
        background: none; border: 1px solid var(--gray); border-radius: var(--radius);
        padding: 4px 12px; font-size: 12px; cursor: pointer;
        transition: all 0.2s ease; color: var(--dark);
    }
    .reorder-btn:hover { background: var(--green); color: var(--dark); border-color: var(--green); }

    .cancel-order-btn {
        background: none; border: 1px solid #d9534f; border-radius: var(--radius);
        padding: 4px 12px; font-size: 12px; cursor: pointer;
        transition: all 0.2s ease; color: #d9534f; font-weight: 600;
    }
    .cancel-order-btn:hover { background: #d9534f; color: #fff; }

    .view-order-btn {
        color: var(--gray-dark); font-size: 12px; text-decoration: none;
        padding: 4px 12px; border: 1px solid var(--gray);
                ...
                <div style="background: #fff; border: 1px solid var(--gray); border-left: 4px solid <?= $statusColor ?>; border-radius: var(--radius); padding: 24px 32px; box-shadow: var(--shadow);">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 16px;">
                        ...
                        <div style="text-align: right;">
                            <p style="font-family: var(--font-heading); font-size: 28px; color: var(--dark);">₱ <?= number_format($order['total_amount'], 2) ?></p>
...
async function cancelUserOrder(orderId) {
    const confirmed = await window.SyncroModal.confirm(
        `Cancel <strong>Order #${String(orderId).padStart(6, '0')}</strong>?<br><br>Reserved products will be returned to stock. A <strong>refund request</strong> will be logged for admin review.`,
        'CANCEL ORDER', 'danger', 'CONFIRM CANCELLATION', 'KEEP ORDER'
    );
    if (!confirmed) return;

    const reason = await window.SyncroModal.prompt(
        'Please share an optional reason for cancelling this order:',
    ...
    try {
        ...
        if (data.success) {
            const refundNote = data.refund_request_id
                ? `<br><br>🔖 <strong>Refund Request #${data.refund_request_id}</strong> is pending admin review.`
                : '';
            await window.SyncroModal.alert(
                (data.message || 'Order cancelled and stock restored.') + refundNote,
