<?php
// pages/about.php - About SYNCRO LAB

session_start();
require_once __DIR__ . '/../database/config.php';
require_once __DIR__ . '/../inc/functions.php';

include __DIR__ . '/../src/Views/layouts/header.php';
?>

<div style="padding: 0; color: var(--dark); background: var(--light);">

    <!-- ============================================
    SECTION 1: HERO (Full Width)
    ============================================ -->
    <section style="position: relative; min-height: 70vh; background: url('/syncro lab/assets/images/workshop-background.jpg') center/cover no-repeat; display: flex; align-items: center; padding: 80px 40px;">
        <div style="position: absolute; inset: 0; background: rgba(18, 23, 27, 0.7); z-index: 1;"></div>
        <div style="position: relative; z-index: 2; max-width: 900px; margin: 0 auto; text-align: center;">
            <h1 style="font-family: var(--font-heading); font-size: 64px; line-height: 1.05; color: var(--green); text-transform: uppercase; margin-bottom: 20px;">
                PRECISION IS NOT<br>AN OPTION.
            </h1>
            <h2 style="font-family: var(--font-heading); font-size: 32px; color: var(--light); text-transform: uppercase; margin-bottom: 16px;">
                IT IS OUR ONLY BENCHMARK.
            </h2>
            <p style="font-size: 20px; color: var(--gray); max-width: 700px; margin: 0 auto 32px;">
                Elite component curation. Precision engineering. Performance solutions.
            </p>
            <a href="/syncro lab/index.php#services" class="btn btn--green" style="font-size: 18px; padding: 14px 40px; transition: transform 0.2s ease;">
                EXPLORE OUR SERVICES →
            </a>
        </div>
    </section>

    <!-- ============================================
    SECTION 2: OUR STORY (Background: var(--light))
    ============================================ -->
    <section style="padding: 80px 40px; background: var(--light);">
        <div style="max-width: 1280px; margin: 0 auto;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 60px; align-items: center;">
                <div>
                    <p style="color: var(--green); font-family: var(--font-heading); font-size: 14px; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 8px;">
                        About Us
                    </p>
                    <h2 style="font-family: var(--font-heading); font-size: 40px; text-transform: uppercase; margin-bottom: 20px;">
                        OUR STORY
                    </h2>
                    <p style="color: var(--gray-dark); font-size: 18px; line-height: 1.8; margin-bottom: 16px;">
                        At SYNCRO LAB, we are a premier retail house that seamlessly blends elite component curation with technical bike fit and maintenance services. Driven by an obsessive dedication to mechanical precision and contemporary performance solutions, we engineer every build, tune, and overhaul to align with your exact vision.
                    </p>
                    <p style="color: var(--gray-dark); font-size: 18px; line-height: 1.8; margin-bottom: 16px;">
                        From custom frame-up assemblies to data-driven biomechanical calibrations, we approach every service with zero margin for error, ensuring maximum power transfer, optimal aerodynamics, and absolute reliability on every ride.
                    </p>
                    <p style="color: var(--gray-dark); font-size: 18px; line-height: 1.8;">
                        Founded on the belief that precision is not an option but a necessity, SYNCRO LAB has grown into a trusted destination for cyclists who demand the best. Our team of master-certified mechanics and fit specialists brings decades of combined experience to every project — from weekend warriors to competitive athletes.
                    </p>
                </div>
                <div style="background: var(--dark); border-radius: var(--radius); padding: 40px; color: var(--light);">
                    <blockquote style="font-family: var(--font-heading); font-size: 24px; line-height: 1.5; color: var(--green); margin: 0 0 16px;">
                        "Precision is not an option — it is our only benchmark."
                    </blockquote>
                    <p style="color: var(--gray); font-size: 16px; line-height: 1.6; margin: 0;">
                        We don't just work on bikes — we partner with riders to understand their goals, their riding style, and their unique biomechanics. This holistic approach ensures every ride is faster, smoother, and more enjoyable.
                    </p>
                </div>
            </div>
        </div>
    </section>

    <!-- ============================================
    SECTION 3: OUR VALUES (Background: var(--dark))
    ============================================ -->
    <section style="padding: 60px 40px 80px; background: var(--dark);">
        <div style="max-width: 1280px; margin: 0 auto;">
            <p style="color: var(--green); font-family: var(--font-heading); font-size: 14px; text-transform: uppercase; letter-spacing: 2px; text-align: center; margin-bottom: 8px;">
                What We Stand For
            </p>
            <h2 style="font-family: var(--font-heading); font-size: 40px; text-transform: uppercase; text-align: center; color: var(--light); margin-bottom: 48px;">
                OUR VALUES
            </h2>
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; margin-bottom: 24px;">
                <!-- Value 1: 0.01MM Torque Tolerance -->
                <div style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius); padding: 32px; text-align: center; transition: transform 0.2s ease, border-color 0.2s ease;">
                    <div style="margin-bottom: 16px;">
                        <svg width="48" height="48" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M89.9932 76.201C89.9823 79.8556 88.5257 83.3573 85.9415 85.9414C83.3574 88.5256 79.8556 89.9822 76.2011 89.9931C73.0612 89.8255 70.1027 88.4695 67.9259 86.2003C67.3351 85.4185 66.6389 84.7223 65.8571 84.1315L38.9626 57.237C35.8612 58.3389 32.5992 58.9214 29.3081 58.961C25.4585 58.9642 21.6459 58.2083 18.0887 56.7365C14.5314 55.2648 11.2993 53.1061 8.57714 50.3839C5.855 47.6618 3.6963 44.4296 2.22455 40.8724C0.752805 37.3151 -0.00313194 33.5026 4.23091e-05 29.6529C-0.0470569 23.7825 1.89778 18.0696 5.51688 13.4473L23.4465 31.3769L31.0321 23.7913L13.4473 6.20641C17.8957 2.42925 23.4776 0.245034 29.3081 0C35.6075 0.00732437 41.7374 2.04151 46.7914 5.80179C51.8454 9.56207 55.5553 14.8488 57.3725 20.8804C59.1897 26.9121 59.0178 33.3683 56.8822 39.2947C54.7467 45.2211 50.7608 50.3029 45.5138 53.789L70.6843 78.9595C71.3739 79.6491 74.4771 82.7523 75.8563 82.7523C77.6836 82.7468 79.4345 82.0185 80.7265 80.7264C82.0186 79.4344 82.7469 77.6835 82.7523 75.8563C82.7726 74.3609 82.2865 72.9027 81.3731 71.7186L79.6491 69.9946L63.0987 53.4442C62.7764 53.1284 62.52 52.7519 62.3442 52.3363C62.1684 51.9208 62.0767 51.4746 62.0744 51.0234C62.0721 50.5723 62.1593 50.1251 62.3309 49.7079C62.5025 49.2906 62.7551 48.9115 63.0741 48.5924C63.3932 48.2734 63.7723 48.0208 64.1896 47.8492C64.6068 47.6776 65.054 47.5904 65.5052 47.5927C65.9563 47.595 66.4025 47.6867 66.8181 47.8625C67.2336 48.0383 67.6101 48.2947 67.9259 48.617L84.4763 65.1674L86.2003 66.8914C88.5767 69.416 89.9287 72.7345 89.9932 76.201ZM52.065 29.6529C52.0709 26.708 51.4953 23.791 50.3711 21.0692C49.2469 18.3473 47.5962 15.8743 45.5139 13.792C43.4316 11.7097 40.9585 10.059 38.2367 8.93481C35.5149 7.81059 32.5978 7.23491 29.653 7.24082C28.7201 7.18486 27.7849 7.30175 26.8945 7.58562L38.273 18.9641C39.9312 20.6735 40.8585 22.9614 40.8585 25.3429C40.8585 27.7244 39.9312 30.0123 38.273 31.7217L31.7218 38.2729C30.0124 39.9311 27.7245 40.8585 25.3429 40.8585C22.9614 40.8585 20.6735 39.9311 18.9641 38.2729L7.58571 26.8945C7.58571 27.9289 7.24089 28.6185 7.24089 29.6529C7.24089 35.5969 9.60216 41.2975 13.8052 45.5006C18.0083 49.7037 23.7089 52.065 29.653 52.065C35.597 52.065 41.2976 49.7037 45.5007 45.5006C49.7038 41.2975 52.065 35.5969 52.065 29.6529Z" fill="#A6CE39"/>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; color: var(--green); text-transform: uppercase; margin-bottom: 8px;">0.01MM TORQUE TOLERANCE</h3>
                    <p style="color: var(--gray); font-size: 15px; line-height: 1.6;">Every bolt, every fitting, every adjustment executed with micron-level precision. We measure twice, torque once.</p>
                </div>

                <!-- Value 2: 100% Direct Factory -->
                <div style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius); padding: 32px; text-align: center; transition: transform 0.2s ease, border-color 0.2s ease;">
                    <div style="margin-bottom: 16px;">
                        <svg width="48" height="48" viewBox="0 0 89 89" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g clip-path="url(#clip0_109_95)">
                            <path d="M77.875 61.4656V47.2812C77.875 46.5436 77.582 45.8362 77.0604 45.3146C76.5388 44.793 75.8314 44.5 75.0938 44.5H58.4063C57.6686 44.5 56.9612 44.793 56.4396 45.3146C55.918 45.8362 55.625 46.5436 55.625 47.2812V61.1875H33.375V47.2812C33.375 46.5436 33.082 45.8362 32.5604 45.3146C32.0388 44.793 31.3314 44.5 30.5938 44.5H13.9063C13.1686 44.5 12.4612 44.793 11.9396 45.3146C11.4181 45.8362 11.125 46.5436 11.125 47.2812V61.4656C7.74538 62.1519 4.74129 64.0694 2.69593 66.846C0.650574 69.6226 -0.290122 73.0601 0.0564591 76.4913C0.403041 79.9225 2.01217 83.1024 4.57148 85.4139C7.13078 87.7254 10.4577 89.0034 13.9063 89H75.0938C78.5424 89.0034 81.8693 87.7254 84.4286 85.4139C86.9879 83.1024 88.597 79.9225 88.9436 76.4913C89.2902 73.0601 88.3495 69.6226 86.3041 66.846C84.2588 64.0694 81.2547 62.1519 77.875 61.4656ZM33.375 66.75C31.5347 69.1421 30.5369 72.0756 30.5369 75.0937C30.5369 78.1119 31.5347 81.0453 33.375 83.4375H24.9757C26.816 81.0453 27.8138 78.1119 27.8138 75.0937C27.8138 72.0756 26.816 69.1421 24.9757 66.75H33.375ZM36.1007 75.0937C36.1007 73.4459 36.5887 71.8349 37.5031 70.464C38.4174 69.0931 39.7173 68.0237 41.2387 67.3906C42.7601 66.7575 44.4348 66.589 46.0518 66.9065C47.6688 67.2239 49.1556 68.0131 50.3247 69.1744C51.4938 70.3358 52.2928 71.8173 52.621 73.4321C52.9492 75.047 52.7919 76.7228 52.1689 78.2484C51.5459 79.7739 50.4851 81.0809 49.1203 82.0043C47.7555 82.9278 46.1479 83.4265 44.5 83.4375C42.2871 83.4375 40.1649 82.5584 38.6001 80.9937C37.0354 79.4289 36.1563 77.3066 36.1563 75.0937H36.1007ZM55.625 66.75H64.08C62.2397 69.1421 61.2419 72.0756 61.2419 75.0937C61.2419 78.1119 62.2397 81.0453 64.08 83.4375H55.625C57.4654 81.0453 58.4632 78.1119 58.4632 75.0937C58.4632 72.0756 57.4654 69.1421 55.625 66.75ZM61.1875 50.0625H72.3125V61.1875H61.1875V50.0625ZM16.6875 50.0625H27.8125V61.1875H16.6875V50.0625ZM5.56253 75.0937C5.56253 73.4435 6.05188 71.8303 6.96871 70.4582C7.88553 69.0861 9.18864 68.0166 10.7133 67.3851C12.2379 66.7536 13.9155 66.5884 15.5341 66.9103C17.1526 67.2322 18.6393 68.0269 19.8062 69.1938C20.9731 70.3607 21.7678 71.8474 22.0897 73.4659C22.4117 75.0845 22.2464 76.7621 21.6149 78.2867C20.9834 79.8114 19.9139 81.1145 18.5418 82.0313C17.1697 82.9481 15.5565 83.4375 13.9063 83.4375C11.6934 83.4375 9.57112 82.5584 8.00636 80.9937C6.4416 79.4289 5.56253 77.3066 5.56253 75.0937ZM75.0938 83.4375C73.4435 83.4375 71.8304 82.9481 70.4582 82.0313C69.0861 81.1145 68.0167 79.8114 67.3852 78.2867C66.7536 76.7621 66.5884 75.0845 66.9103 73.4659C67.2323 71.8474 68.027 70.3607 69.1939 69.1938C70.3607 68.0269 71.8475 67.2322 73.466 66.9103C75.0845 66.5884 76.7622 66.7536 78.2868 67.3851C79.8114 68.0166 81.1145 69.0861 82.0314 70.4582C82.9482 71.8303 83.4375 73.4435 83.4375 75.0937C83.4375 77.3066 82.5585 79.4289 80.9937 80.9937C79.4289 82.5584 77.3067 83.4375 75.0938 83.4375ZM11.125 38.9375C11.8627 38.9375 12.5701 38.6445 13.0917 38.1229C13.6133 37.6013 13.9063 36.8939 13.9063 36.1562C13.9063 35.4186 13.6133 34.7112 13.0917 34.1896C12.5701 33.668 11.8627 33.375 11.125 33.375C10.0249 33.375 8.94942 33.0487 8.03467 32.4375C7.11992 31.8263 6.40696 30.9576 5.98595 29.9412C5.56494 28.9247 5.45478 27.8063 5.66941 26.7273C5.88404 25.6483 6.41382 24.6571 7.19175 23.8792C7.96968 23.1013 8.96082 22.5715 10.0398 22.3569C11.1189 22.1422 12.2373 22.2524 13.2537 22.6734C14.2701 23.0944 15.1389 23.8074 15.7501 24.7221C16.3613 25.6369 16.6875 26.7123 16.6875 27.8125C16.6875 28.5501 16.9806 29.2575 17.5021 29.7791C18.0237 30.3007 18.7311 30.5937 19.4688 30.5937C20.2064 30.5937 20.9138 30.3007 21.4354 29.7791C21.957 29.2575 22.25 28.5501 22.25 27.8125C22.25 24.8619 21.0779 22.0323 18.9916 19.9459C16.9053 17.8596 14.0756 16.6875 11.125 16.6875V11.125H33.375C33.375 14.0755 34.5471 16.9052 36.6335 18.9915C38.7198 21.0779 41.5495 22.25 44.5 22.25C47.4506 22.25 50.2802 21.0779 52.3666 18.9915C54.4529 16.9052 55.625 14.0755 55.625 11.125H77.875V16.6875C74.9245 16.6875 72.0948 17.8596 70.0085 19.9459C67.9221 22.0323 66.75 24.8619 66.75 27.8125C66.75 28.5501 67.0431 29.2575 67.5646 29.7791C68.0862 30.3007 68.7936 30.5937 69.5313 30.5937C70.2689 30.5937 70.9763 30.3007 71.4979 29.7791C72.0195 29.2575 72.3125 28.5501 72.3125 27.8125C72.3125 26.7123 72.6388 25.6369 73.25 24.7221C73.8612 23.8074 74.7299 23.0944 75.7463 22.6734C76.7628 22.2524 77.8812 22.1422 78.9602 22.3569C80.0392 22.5715 81.0304 23.1013 81.8083 23.8792C82.5862 24.6571 83.116 25.6483 83.3306 26.7273C83.5453 27.8063 83.4351 28.9247 83.0141 29.9412C82.5931 30.9576 81.8801 31.8263 80.9654 32.4375C80.0506 33.0487 78.9752 33.375 77.875 33.375C77.1374 33.375 76.43 33.668 75.9084 34.1896C75.3868 34.7112 75.0938 35.4186 75.0938 36.1562C75.0938 36.8939 75.3868 37.6013 75.9084 38.1229C76.43 38.6445 77.1374 38.9375 77.875 38.9375C80.3029 38.9127 82.656 38.0943 84.5753 36.6072C86.4947 35.1201 87.8748 33.0459 88.5051 30.7011C89.1354 28.3563 88.9813 25.8697 88.0663 23.6207C87.1514 21.3717 85.5257 19.4838 83.4375 18.245V8.34373C83.4375 7.6061 83.1445 6.89868 82.6229 6.37709C82.1013 5.8555 81.3939 5.56248 80.6563 5.56248H54.0675C53.0825 3.90238 51.6824 2.52715 50.005 1.57197C48.3275 0.616779 46.4304 0.114502 44.5 0.114502C42.5697 0.114502 40.6725 0.616779 38.9951 1.57197C37.3176 2.52715 35.9176 3.90238 34.9325 5.56248H8.34378C7.60615 5.56248 6.89873 5.8555 6.37714 6.37709C5.85555 6.89868 5.56253 7.6061 5.56253 8.34373V18.245C3.47431 19.4838 1.84869 21.3717 0.933723 23.6207C0.0187524 25.8697 -0.135331 28.3563 0.494985 30.7011C1.1253 33.0459 2.5054 35.1201 4.42471 36.6072C6.34402 38.0943 8.69714 38.9127 11.125 38.9375ZM44.5 5.56248C45.6002 5.56248 46.6756 5.88872 47.5904 6.49993C48.5051 7.11115 49.2181 7.97989 49.6391 8.9963C50.0601 10.0127 50.1703 11.1312 49.9556 12.2102C49.741 13.2892 49.2112 14.2803 48.4333 15.0583C47.6554 15.8362 46.6642 16.366 45.5852 16.5806C44.5062 16.7952 43.3878 16.6851 42.3714 16.2641C41.3549 15.843 40.4862 15.1301 39.875 14.2153C39.2638 13.3006 38.9375 12.2251 38.9375 11.125C38.9375 9.64971 39.5236 8.23487 40.5667 7.1917C41.6099 6.14853 43.0248 5.56248 44.5 5.56248Z" fill="#A6CE39"/>
                            </g>
                            <defs>
                                <clipPath id="clip0_109_95">
                                    <rect width="89" height="89" fill="white"/>
                                </clipPath>
                            </defs>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; color: var(--green); text-transform: uppercase; margin-bottom: 8px;">100% DIRECT FACTORY</h3>
                    <p style="color: var(--gray); font-size: 15px; line-height: 1.6;">Authentic components sourced directly from the world's leading manufacturers, guaranteeing quality and authenticity.</p>
                </div>

                <!-- Value 3: 24-48 HR Turnaround -->
                <div style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius); padding: 32px; text-align: center; transition: transform 0.2s ease, border-color 0.2s ease;">
                    <div style="margin-bottom: 16px;">
                        <svg width="48" height="48" viewBox="0 0 87 87" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g clip-path="url(#clip0_109_93)">
                            <path d="M32.625 86.9999H3.625C2.66359 86.9999 1.74156 86.618 1.06174 85.9381C0.381919 85.2583 0 84.3363 0 83.3749C0 82.4135 0.381919 81.4914 1.06174 80.8116C1.74156 80.1318 2.66359 79.7499 3.625 79.7499H32.625C33.5864 79.7499 34.5084 80.1318 35.1883 80.8116C35.8681 81.4914 36.25 82.4135 36.25 83.3749C36.25 84.3363 35.8681 85.2583 35.1883 85.9381C34.5084 86.618 33.5864 86.9999 32.625 86.9999ZM25.375 72.4999H3.625C2.66359 72.4999 1.74156 72.118 1.06174 71.4381C0.381919 70.7583 0 69.8363 0 68.8749C0 67.9135 0.381919 66.9914 1.06174 66.3116C1.74156 65.6318 2.66359 65.2499 3.625 65.2499H25.375C26.3364 65.2499 27.2584 65.6318 27.9383 66.3116C28.6181 66.9914 29 67.9135 29 68.8749C29 69.8363 28.6181 70.7583 27.9383 71.4381C27.2584 72.118 26.3364 72.4999 25.375 72.4999ZM18.125 57.9999H3.625C2.66359 57.9999 1.74156 57.618 1.06174 56.9381C0.381919 56.2583 0 55.3363 0 54.3749C0 53.4135 0.381919 52.4914 1.06174 51.8116C1.74156 51.1318 2.66359 50.7499 3.625 50.7499H18.125C19.0864 50.7499 20.0084 51.1318 20.6883 51.8116C21.3681 52.4914 21.75 53.4135 21.75 54.3749C21.75 55.3363 21.3681 56.2583 20.6883 56.9381C20.0084 57.618 19.0864 57.9999 18.125 57.9999ZM47.125 86.8368C46.1636 86.8795 45.2246 86.5387 44.5145 85.8891C43.8044 85.2395 43.3815 84.3345 43.3387 83.3731C43.2959 82.4117 43.6368 81.4726 44.2864 80.7626C44.9359 80.0525 45.841 79.6295 46.8024 79.5868C53.6625 78.9591 60.2018 76.3896 65.6543 72.1794C71.1068 67.9691 75.2467 62.2924 77.5892 55.8141C79.9317 49.3358 80.3798 42.3241 78.8809 35.6003C77.382 28.8765 73.9983 22.719 69.1261 17.849C64.2538 12.979 58.0948 9.59815 51.3703 8.10238C44.6458 6.60662 37.6344 7.0579 31.1571 9.40336C24.6799 11.7488 19.0051 15.8914 14.7973 21.3458C10.5896 26.8002 8.02316 33.3407 7.39862 40.2011C7.35578 40.6753 7.21997 41.1363 6.99894 41.558C6.77791 41.9796 6.476 42.3536 6.11044 42.6586C5.74488 42.9636 5.32283 43.1935 4.86839 43.3354C4.41395 43.4773 3.93601 43.5282 3.46187 43.4854C2.98774 43.4425 2.52668 43.3067 2.10503 43.0857C1.68338 42.8647 1.30939 42.5628 1.00442 42.1972C0.699452 41.8316 0.46947 41.4096 0.327607 40.9551C0.185745 40.5007 0.134781 40.0228 0.177625 39.5486C1.1886 28.3956 6.46194 18.0622 14.8999 10.6993C23.3379 3.33646 34.2905 -0.488519 45.4776 0.0206032C56.6648 0.529726 67.2246 5.33372 74.959 13.4326C82.6933 21.5314 87.0062 32.3012 87 43.4999C87.0556 54.3689 83.0177 64.8605 75.6896 72.8878C68.3616 80.9151 58.2803 85.8898 47.4513 86.8223C47.3425 86.8331 47.2301 86.8368 47.125 86.8368ZM43.5 21.7499C42.5386 21.7499 41.6166 22.1318 40.9367 22.8116C40.2569 23.4914 39.875 24.4135 39.875 25.3749V43.4999C39.8752 44.4612 40.2573 45.3831 40.9371 46.0628L51.8121 56.9378C52.4958 57.5981 53.4115 57.9635 54.362 57.9552C55.3124 57.9469 56.2216 57.5657 56.8937 56.8936C57.5658 56.2215 57.9471 55.3123 57.9553 54.3618C57.9636 53.4114 57.5982 52.4957 56.9379 51.812L47.125 41.9991V25.3749C47.125 24.4135 46.7431 23.4914 46.0633 22.8116C45.3834 22.1318 44.4614 21.7499 43.5 21.7499Z" fill="#A6CE39"/>
                            </g>
                            <defs>
                                <clipPath id="clip0_109_93">
                                    <rect width="87" height="87" fill="white"/>
                                </clipPath>
                            </defs>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; color: var(--green); text-transform: uppercase; margin-bottom: 8px;">24-48 HOUR TURNAROUND</h3>
                    <p style="color: var(--gray); font-size: 15px; line-height: 1.6;">Fast, reliable service without ever compromising on quality. We respect your time as much as your bike.</p>
                </div>
            </div>

            <!-- Row 2: 2 Values -->
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; max-width: 700px; margin: 0 auto;">
                <div style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius); padding: 32px; text-align: center; transition: transform 0.2s ease, border-color 0.2s ease;">
                    <div style="margin-bottom: 16px;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" fill="#A6CE39" stroke="#A6CE39" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; color: var(--green); text-transform: uppercase; margin-bottom: 8px;">ELITE CURATION</h3>
                    <p style="color: var(--gray); font-size: 15px; line-height: 1.6;">Hand-selected components from the world's most respected brands. Because your bike deserves the best.</p>
                </div>

                <div style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius); padding: 32px; text-align: center; transition: transform 0.2s ease, border-color 0.2s ease;">
                    <div style="margin-bottom: 16px;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M21 12C21 13.2 20.5 14.2 19.7 15.1C18.9 15.9 17.8 16.5 16.5 16.8C15.2 17.1 13.8 17.1 12.2 16.8C10.6 16.5 9.1 15.9 7.9 15.1M3 12C3 13.2 3.5 14.2 4.3 15.1C5.1 15.9 6.2 16.5 7.5 16.8C8.8 17.1 10.2 17.1 11.8 16.8C13.4 16.5 14.9 15.9 16.1 15.1" stroke="#A6CE39" stroke-width="2" stroke-linecap="round"/>
                            <path d="M12 2L12 22" stroke="#A6CE39" stroke-width="2" stroke-linecap="round"/>
                            <path d="M6 8L12 12L18 8" stroke="#A6CE39" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M6 12L12 16L18 12" stroke="#A6CE39" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; color: var(--green); text-transform: uppercase; margin-bottom: 8px;">BIOMECHANICAL EXCELLENCE</h3>
                    <p style="color: var(--gray); font-size: 15px; line-height: 1.6;">Data-driven bike fits backed by motion analysis, power-transfer telemetry, and biomechanical expertise.</p>
                </div>
            </div>
        </div>
    </section>

    <!-- ============================================
    SECTION 4: OUR PHILOSOPHY (Background: var(--light))
    ============================================ -->
    <section style="padding: 80px 40px; background: var(--light);">
        <div style="max-width: 1280px; margin: 0 auto; text-align: center;">
            <p style="color: var(--green); font-family: var(--font-heading); font-size: 14px; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 8px;">
                Our Philosophy
            </p>
            <h2 style="font-family: var(--font-heading); font-size: 36px; text-transform: uppercase; margin-bottom: 20px;">
                MORE THAN A BIKE SHOP
            </h2>
            <blockquote style="font-size: 24px; color: var(--dark); line-height: 1.6; margin: 0 auto 32px; max-width: 850px;">
                "We don't just build bikes — we engineer experiences. Every component is chosen with purpose, every adjustment is made with precision, and every ride is optimized for performance."
            </blockquote>
            <p style="color: var(--gray-dark); font-size: 18px; line-height: 1.8; max-width: 800px; margin: 0 auto;">
                Our philosophy is simple: take the time to get it right. Whether we're assembling a custom build from the frame up or performing a routine tune-up, we treat every bike like it's our own. We believe that excellence is not a destination but a continuous pursuit — one that requires dedication, expertise, and a relentless commitment to quality.
            </p>
            <div style="margin-top: 32px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; max-width: 750px; margin-left: auto; margin-right: auto;">
                <div>
                    <h3 style="font-family: var(--font-heading); font-size: 16px; color: var(--dark); text-transform: uppercase; margin-bottom: 4px;">Dedication</h3>
                    <p style="font-size: 14px; color: var(--gray-dark);">To the craft and to our riders.</p>
                </div>
                <div>
                    <h3 style="font-family: var(--font-heading); font-size: 16px; color: var(--dark); text-transform: uppercase; margin-bottom: 4px;">Expertise</h3>
                    <p style="font-size: 14px; color: var(--gray-dark);">Backed by years of experience.</p>
                </div>
                <div>
                    <h3 style="font-family: var(--font-heading); font-size: 16px; color: var(--dark); text-transform: uppercase; margin-bottom: 4px;">Quality</h3>
                    <p style="font-size: 14px; color: var(--gray-dark);">Uncompromising in every detail.</p>
                </div>
            </div>
        </div>
    </section>

    <!-- ============================================
    SECTION 5: WHY CHOOSE SYNCRO LAB (Background: var(--dark))
    ============================================ -->
    <section style="padding: 60px 40px; background: var(--dark);">
        <div style="max-width: 1280px; margin: 0 auto;">
            <p style="color: var(--green); font-family: var(--font-heading); font-size: 14px; text-transform: uppercase; letter-spacing: 2px; text-align: center; margin-bottom: 8px;">
                Why Choose Us
            </p>
            <h2 style="font-family: var(--font-heading); font-size: 40px; text-transform: uppercase; text-align: center; color: var(--light); margin-bottom: 48px;">
                WHY SYNCRO LAB
            </h2>
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px;">
                <div style="background: var(--light); border-radius: var(--radius); border: 1px solid var(--gray); padding: 32px; text-align: center; box-shadow: var(--shadow); transition: transform 0.2s ease;">
                    <div style="font-size: 36px; margin-bottom: 12px;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#A6CE39" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 2C11.5 4 10 6 8 8C6 10 4 11.5 2 12C4 12.5 6 14 8 16C10 18 11.5 20 12 22C12.5 20 14 18 16 16C18 14 20 12.5 22 12C20 11.5 18 10 16 8C14 6 12.5 4 12 2Z" stroke="currentColor" fill="none" stroke-linejoin="round"/>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; text-transform: uppercase; margin-bottom: 8px;">Master-Certified Mechanics</h3>
                    <p style="color: var(--gray-dark); font-size: 15px; line-height: 1.6;">Our team holds certifications from leading brands and training institutions worldwide.</p>
                </div>
                <div style="background: var(--light); border-radius: var(--radius); border: 1px solid var(--gray); padding: 32px; text-align: center; box-shadow: var(--shadow); transition: transform 0.2s ease;">
                    <div style="font-size: 36px; margin-bottom: 12px;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#A6CE39" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
                            <rect x="3" y="11" width="18" height="11" rx="2" stroke="currentColor"/>
                            <path d="M7 11V7C7 5 8 4 10 4H14C16 4 17 5 17 7V11" stroke="currentColor"/>
                            <path d="M12 16V19" stroke="currentColor" stroke-width="2"/>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; text-transform: uppercase; margin-bottom: 8px;">Workmanship Guarantee</h3>
                    <p style="color: var(--gray-dark); font-size: 15px; line-height: 1.6;">We stand behind every service we perform. Follow-up adjustments and expert support are always included.</p>
                </div>
                <div style="background: var(--light); border-radius: var(--radius); border: 1px solid var(--gray); padding: 32px; text-align: center; box-shadow: var(--shadow); transition: transform 0.2s ease;">
                    <div style="font-size: 36px; margin-bottom: 12px;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#A6CE39" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="12" cy="12" r="9" stroke="currentColor"/>
                            <path d="M12 8V12L14 14" stroke="currentColor"/>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; text-transform: uppercase; margin-bottom: 8px;">Premium Brands</h3>
                    <p style="color: var(--gray-dark); font-size: 15px; line-height: 1.6;">We stock and work with only the best: Shimano, SRAM, Campagnolo, Specialized, Canyon, and more.</p>
                </div>
                <div style="background: var(--light); border-radius: var(--radius); border: 1px solid var(--gray); padding: 32px; text-align: center; box-shadow: var(--shadow); transition: transform 0.2s ease;">
                    <div style="font-size: 36px; margin-bottom: 12px;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#A6CE39" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
                            <rect x="4" y="6" width="16" height="14" rx="2" stroke="currentColor"/>
                            <path d="M8 6V4H16V6" stroke="currentColor"/>
                            <line x1="12" y1="11" x2="12" y2="17" stroke="currentColor"/>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; text-transform: uppercase; margin-bottom: 8px;">Transparent Pricing</h3>
                    <p style="color: var(--gray-dark); font-size: 15px; line-height: 1.6;">No hidden fees. Clear, transparent estimates provided after a thorough diagnostic assessment.</p>
                </div>
                <div style="background: var(--light); border-radius: var(--radius); border: 1px solid var(--gray); padding: 32px; text-align: center; box-shadow: var(--shadow); transition: transform 0.2s ease;">
                    <div style="font-size: 36px; margin-bottom: 12px;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#A6CE39" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
                            <path d="M4 4H20V20H4Z" stroke="currentColor"/>
                            <path d="M4 12H20" stroke="currentColor"/>
                            <path d="M12 4V20" stroke="currentColor"/>
                            <path d="M8 12L8 20" stroke="currentColor"/>
                            <path d="M16 12L16 20" stroke="currentColor"/>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; text-transform: uppercase; margin-bottom: 8px;">Authorized Service Center</h3>
                    <p style="color: var(--gray-dark); font-size: 15px; line-height: 1.6;">As an accredited shop, we handle manufacturer warranty claims for products purchased through us.</p>
                </div>
                <div style="background: var(--light); border-radius: var(--radius); border: 1px solid var(--gray); padding: 32px; text-align: center; box-shadow: var(--shadow); transition: transform 0.2s ease;">
                    <div style="font-size: 36px; margin-bottom: 12px;">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#A6CE39" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" stroke="currentColor"/>
                            <path d="M12 8L12 13" stroke="currentColor"/>
                            <path d="M12 16V16.5" stroke="currentColor"/>
                        </svg>
                    </div>
                    <h3 style="font-family: var(--font-heading); font-size: 20px; text-transform: uppercase; margin-bottom: 8px;">Community Focused</h3>
                    <p style="color: var(--gray-dark); font-size: 15px; line-height: 1.6;">We're riders too. We sponsor local events, group rides, and cycling initiatives in our community.</p>
                </div>
            </div>
        </div>
    </section>

    <!-- ============================================
    SECTION 6: CTA (Background: var(--light) with Green Border)
    ============================================ -->
    <section style="background: var(--light); border-top: 2px solid var(--green); padding: 64px 40px; text-align: center;">
        <div style="max-width: 1280px; margin: 0 auto;">
            <h2 style="font-family: var(--font-heading); font-size: 36px; color: var(--dark); text-transform: uppercase; margin-bottom: 12px;">
                Ready to Experience the<br><span style="color: var(--green);">SYNCRO LAB Difference?</span>
            </h2>
            <p style="color: var(--gray-dark); font-size: 18px; margin-bottom: 32px;">
                Visit one of our branches or book a service today.
            </p>
            <div style="display: flex; gap: 16px; justify-content: center; flex-wrap: wrap;">
                <a href="/syncro lab/pages/booking.php" class="btn btn--green" style="font-size: 18px; padding: 14px 40px; transition: transform 0.2s ease;">
                    BOOK A SERVICE →
                </a>
                <a href="/syncro lab/pages/shop.php" class="btn btn--outline" style="font-size: 18px; padding: 14px 40px; transition: transform 0.2s ease;">
                    BROWSE OUR SHOP
                </a>
            </div>
        </div>
    </section>

</div>

<?php include __DIR__ . '/../src/Views/layouts/footer.php'; ?>